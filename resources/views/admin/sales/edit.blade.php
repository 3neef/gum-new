    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sales.update", [$sale->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id" required>
                    @foreach($customers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $sale->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('phones') ? 'is-invalid' : '' }}" name="phones[]" id="phones" multiple required>
                    @foreach($phones as $id => $phone)
                        <option value="{{ $id }}" {{ (in_array($id, old('phones', [])) || $sale->phones->contains($id)) ? 'selected' : '' }}>{{ $phone }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Sale::OPERATION_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('operation') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="operation_{{ $key }}" name="operation" value="{{ $key }}" {{ old('operation', $sale->operation) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{!! old('notes', $sale->notes) !!}</textarea>
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_price') ? 'is-invalid' : '' }}" type="text" name="total_price" id="total_price" value="{{ old('total_price', $sale->total_price) }}" required>
