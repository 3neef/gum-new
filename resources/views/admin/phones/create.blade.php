    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.phones.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Phone::STATE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('state') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="state_{{ $key }}" name="state" value="{{ $key }}" {{ old('state', 'new') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('battery') ? 'is-invalid' : '' }}" type="number" name="battery" id="battery" value="{{ old('battery', '') }}" step="1">
