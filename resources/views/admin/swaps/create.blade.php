...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.swap.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.swaps.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('phone_1') ? 'is-invalid' : '' }}" name="phone_1_id" id="phone_1_id" required>
                    @foreach($phone_1s as $id => $entry)
                        <option value="{{ $id }}" {{ old('phone_1_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('phone_2') ? 'is-invalid' : '' }}" name="phone_2_id" id="phone_2_id" required>
                    @foreach($phone_2s as $id => $entry)
                        <option value="{{ $id }}" {{ old('phone_2_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price_diffrance') ? 'is-invalid' : '' }}" type="text" name="price_diffrance" id="price_diffrance" value="{{ old('price_diffrance', '') }}">
